...
function CountUp({ target, prefix, unit }: { target: number; prefix?: string; unit?: string }) {
  ...
  return (
    <Text
      as="div"
      fontFamily="display"
      className="text-[clamp(32px,4vw,46px)] font-semibold tracking-[-0.02em] leading-none flex items-baseline gap-0.75 whitespace-nowrap"
    >
...
function StatItem({ prefix, target, unit, label, method, context }: (typeof STATS)[number]) {
  return (
    <div className="py-8 px-6.5 border-r border-border last:border-r-0 max-tablet:nth-2:border-r-0 max-tablet:nth-1:border-b max-tablet:nth-2:border-b">
...
export function Stats() {
  return (
    <section className="py-0 pb-30 max-compact:pb-21">
      <div className="max-w-280 mx-auto px-8 w-full max-compact:px-5">
        <Reveal>
          <div className="grid grid-cols-4 border border-border rounded-2xl overflow-hidden bg-surface max-tablet:grid-cols-2">
            {STATS.map((stat) => (
