...
탐색 끝에 선택한 게 CASL이다.  
이 글은 ABAC 시리즈(1~3편)에서 사용한 CASL의 핵심 개념을 별도로 정리한 레퍼런스다.  
...
`course-7`은 허용되지만 `course-8`은 Condition에 맞지 않아 false다.  
...
이 Deny 우선 원칙 때문에 규칙 등록 순서가 중요하다.  
Allow를 먼저 전부 등록하고, Deny를 나중에 등록해야 Deny가 의도한 대로 Allow를 덮어쓴다.  
GEM에서는 이 원칙을 `AbilityFactory` 설계에 직접 반영했다. 상세 구현은 ABAC 시리즈 2편에서 다룬다.  
...
Level 기반 Allow가 먼저 등록되고, PolicyOverride Deny가 나중에 등록되어 어떤 Allow 규칙도 덮어쓴다.  
이 동작이 보장되려면 CASL이 뒤에 등록된 규칙을 우선한다는 특성을 이용해야 한다.  
...
왜 두 단계로 나눠야 하는지, Guard에서 전부 처리하면 안 되는지에 대한 상세 설명은 ABAC 시리즈 2편에서 다룬다.  
