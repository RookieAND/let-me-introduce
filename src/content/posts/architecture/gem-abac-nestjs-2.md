...
지금은 단순함을 유지하는 쪽을 선택했고, 이 결정이 틀렸다고는 생각하지 않는다. 완벽한 해법보다 현재 상황에 맞는 해법이 더 중요하다.  

Guard에서 빌드한 Ability를 Service까지 전달하는 데 `nestjs-cls` (AsyncLocalStorage 기반)를 사용했다. 요청마다 독립된 저장소가 생기므로 요청 간 오염이 없다.  
`nestjs-cls`가 없었다면 Ability를 Guard에서 Service로 어떻게 넘길지가 꽤 골치 아픈 문제였을 것이다. 요청 컨텍스트를 Provider 간에 공유하는 가장 깔끔한 방법이었다.  
