...
Guard는 컨트롤러보다 먼저 실행된다. 즉, Guard 시점에는 아직 Service 로직이 수행되지 않았고, 실제 리소스 객체도 없다.  
그러니까 Guard에서 리소스를 꺼내려면 DB를 한 번 더 조회해야 한다는 얘기인데, Service에서도 어차피 같은 걸 조회할 텐데 이건 명백한 중복이다. (으악 이건 아니야)
...
`ResourcePermission`은 사용자와 리소스 간의 권한 관계를 저장한다. 일반 사용자가 Space에 초대되거나 과제에 접근 권한을 부여받을 때마다 레코드가 생긴다. UI를 통해 자동으로 만들어지고, 변경도 잦다.
...
반면 `PolicyOverride`는 유저별 예외 규칙을 저장한다. `rules` 배열 안에 CASL PolicyRule이 직접 담기고, 생성과 수정은 개발자가 직접 한다. 일반 UI에는 노출되지 않는다.
...
`expandLevelToRules` 안에서 Space ADMIN 정적 상속도 함께 처리한다. Space ADMIN이면 하위 Course/Unit/Submission에도 ADMIN PolicyRule을 추가 생성한다.
...
그런데 이 등록 과정에서 한 가지 더 처리해야 할 게 있다. `resources: ['course-7']`는 그대로 CASL에 전달할 수 없다. MongoDB 조건 형식으로 변환이 필요하다.
...
캐싱 없이 매 요청마다 빌드하는 구조이므로, CLS를 통한 단순한 전달만으로 충분했다. `AbilityClsService`는 CLS에 Ability를 넣고 꺼내는 역할만 한다.
...
실행 흐름 외에 타입 안전성도 챙겼다. `SubjectActionMap` 타입 덕분에 잘못된 action-subject 조합은 컴파일 에러가 난다.
...
`assertCan`이 false이면 `ForbiddenException`이 자동으로 발생한다. PolicyOverride Deny가 걸린 경우도 이 경로로 차단된다.
