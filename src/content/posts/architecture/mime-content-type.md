...
-   인터넷을 통해 전송 가능한 문자는 오로지 **ASCII** 표준이기 때문에, 다른 데이터를 이로 변환하는 과정이 필요해졌음.
...
-   게다가 `MIME` 의 경우 전송한 파일의 포맷에 대한 정보도 제공했기에, 더욱 큰 이점이 있다.

#### 2-1. 바이너리 데이터를 왜 인코딩 해야 했을까?

-   ASCII 의 경우 제어 문자에 `0 ~ 32` 번을 할당시켰다, 제어 문자의 경우 폰트의 변경이나 전송의 끝을 알리는 문자처럼 특수한 작업을 하기에 중요한 요소이다.
