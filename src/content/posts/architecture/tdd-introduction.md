...
> 프로게이머 **BDD**는 들어봤는데 TDD는 처음 들어봅니다...  

Jest, React-Testing-Library에 대한 학습을 최근 진행 중인데, 처음에는 대체 이것들을 왜 굳이 일일히 테스트 해야 하는지에 대한 귀찮음이 먼저 들었다. 지금까지 나는 상당히 원시적인 방법으로 개발 테스트를 진행하고 있었는데... 코드에 `console.log` 를 추가하여 결과값을 콘솔에서 확인한다던가 하는 식이었다. 지금 생각해도 이러한 방식은 디버깅을 매우 어렵게 만들며 코드 수정에 걸리는 시간도 늘리는 주범이었다.  

따라서 이번 기회에 테스팅 라이브러리를 배울 겸, 대체 TDD가 무엇인지도 궁금하여 개인적인 학습을 추가로 진행하였다. 앞으로 실무에서 테스트 케이스를 추가하고 직접 테스트 코드를 작성할 일이 생길지도 모르는데 미리미리 준비하는게 좋지 않겠는가? 여튼, 한번 TDD를 파헤쳐보자.  

해당 포스트는 `Udemy` 의 `Jest 및 테스팅 라이브러리로 React 테스트하기 강의` 를 상당 부분 참고하였다. 좋은 강의라고 생각하니 관심이 있다면 한번 들어보자.  
...
-   이를 **red-green 테스트** 라고도 하는데, 코드 작성 전에 실패하는 테스트 코드 `(레드)` 를 작성하고, 코드 작성 후에는 테스트가 통과 `(그린)` 하는지를 확인하는 것이다.
...
-   중요한 것은 실패하는 테스트 코드를 작성하기 전까지는 개발에 착수하지 않아야 한다.
...
-   결국 이는 코드의 재사용성을 떨어트리고, 관리가 어려워지며 유지보수를 어렵게 하는 원인이 된다.
-   또한 사소한 코드 수정에도 애플리케이션의 모든 부분에 대한 테스트가 진행되어야 하기에 버그의 원인을 찾기가 어려워진다.
