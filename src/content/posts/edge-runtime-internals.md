...
`vm` 모듈은 V8 가상 머신 컨텍스트 내에서 스크립트를 컴파일하고 실행 가능하도록 한다. **샌드박스 환경**으로 코드를 실행해 보다 안전한 환경에서 검증되지 않은 코드를 실행할 수 있게 한다.

이 모듈을 사용할 경우 다른 V8 엔진 구현체(Context) 내에서 코드를 실행할 수 있다. 각 실행된 코드들이 고유한 스코프를 가지며, 전역 변수를 서로 공유하지 않는다.
