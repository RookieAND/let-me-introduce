...
- Variable Object (VO) 란, 실행 컨텍스트 내에서 생성되는 객체와 유사한 컨테이너다.  
  내부에는 실행 컨텍스트 내부에서 정의된 변수와 함수의 선언부를 담고 있다.  
...
- 따라서 함수 선언문으로 생성된 함수들은 코드 실행 전에 이미 VO에 추가된 상태이기에, 해당 함수가 선언된 장소 이전에도 사용이 가능하다.  
  이러한 현상을 **호이스팅** 이라고 한다.  

2. Scope Chain 생성

- Scope Chain 이란 현재 실행 컨텍스트에서 접근 가능한 변수와 함수들의 범위를 결정하는 체인 구조다.
- 각 실행 컨텍스트는 Outer Environment Reference 를 통해 상위 스코프의 Variable Object 를 참조하며, 이 참조들이 연결된 것을 Scope Chain 이라 한다.
- 변수를 탐색할 때 현재 컨텍스트의 VO 에서 찾지 못하면 Scope Chain 을 따라 상위 컨텍스트의 VO 를 순차적으로 탐색한다.  
  최종적으로 전역 실행 컨텍스트까지 탐색했음에도 찾지 못하면 `ReferenceError` 가 발생한다.  
- 이 과정 덕분에 내부 함수에서 외부 함수의 변수에 접근할 수 있는 **클로저 (Closure)** 가 가능해진다.

3. this 바인딩

- `this` 는 실행 컨텍스트가 생성될 때 결정되며, **함수가 어떻게 호출되었는지** 에 따라 가리키는 대상이 달라진다.
- 전역 실행 컨텍스트에서 `this` 는 전역 객체를 가리킨다.  
  브라우저에서는 `window`, Node.js 에서는 `global` 이다.  
- 함수 실행 컨텍스트에서는 호출 방식에 따라 `this` 가 결정된다.
  - 일반 함수 호출 시 전역 객체 (strict mode 에서는 `undefined`)
