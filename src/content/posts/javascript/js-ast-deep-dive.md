...
코드가 어떤 과정을 거쳐 트리가 되는지, 그 트리의 각 노드가 어떤 구조를 갖는지, 그리고 Babel은 그 트리를 어떻게 순회하는지. 이번 글에서 이를 기록하고자 한다!
...
여기서 굳이 "추상" 이라는 단어가 붙은 이유는 소스 코드의 모든 세부사항을 담지 않기 때문이다. 
...
가장 먼저 `Program`이 루트다. 그 아래 `VariableDeclaration`이 있고, 그 안에 `VariableDeclarator`가 있다. 선언된 변수 이름은 `Identifier`, 초기값은 `BinaryExpression`으로 표현된다. 
...
함수는 정의 방식에 따라 크게 세 타입으로 갈린다. 

`function foo() {}` 형식은 `FunctionDeclaration`, `const foo = function() {}` 형식은 `FunctionExpression`, `const foo = () => {}` 형식은 `ArrowFunctionExpression`이다. 

런타임에서는 동작이 거의 같지만 AST 위에서는 완전히 다른 노드로 존재한다. 그렇기에 Babel 플러그인이 화살표 함수를 일반 함수로 변환할 때 이 구분이 중요해진다.

값과 이름을 다루는 노드도 구분된다. 변수명, 함수명처럼 이름을 가리키는 식별자는 `Identifier`이고 `name` 필드에 실제 이름이 담긴다. 리터럴 값은 파서마다 표현 방식이 다른데, Babel은 `NumericLiteral`, `StringLiteral`처럼 세분화하고 estree 스펙은 `Literal` 하나로 통합한다.

연산과 호출은 각자 고유한 구조를 가진다. `+`, `-` 같은 이항 연산자는 `BinaryExpression`으로 표현되며 `operator`, `left`, `right` 필드를 갖는다. 함수 호출은 `CallExpression`인데, `callee`(호출되는 함수)와 `arguments`(인자 목록)로 구성된다. `obj.prop` 같은 속성 접근은 `MemberExpression`이고, `object`와 `property` 필드가 있다. 번들러가 모듈 그래프를 분석할 때는 `ImportDeclaration`과 `ExportDeclaration`을 탐색한다.
...
AST를 변환하거나 분석하려면 트리 전체를 순회해야 한다. 가장 기본적인 방식은 **깊이 우선 탐색(DFS, Depth-First Search)**이다.

루트 노드에서 시작해서 자식 노드로 내려가고, 더 이상 자식이 없으면 형제 노드로 이동한다. 재귀 함수로 구현하면 직관적이다.
...
이 함수는 노드를 방문할 때마다 `node.type`을 출력한다. 하지만 단순 DFS만으로는 "특정 유형의 노드를 만났을 때 특정 작업을 한다"는 요구를 깔끔하게 처리하기 어렵다. 유형마다 분기 로직이 함수 안에 뒤섞이기 때문이다.

Babel이 채택한 해법이 **Visitor 패턴**이다. 탐색 로직과 처리 로직을 분리한다. 탐색은 Babel 내부가 담당하고, 외부에서는 어떤 노드 유형에 어떤 작업을 할지만 선언한다.
...
각 키는 노드 유형 이름이고, 값은 그 노드를 방문했을 때 실행할 함수다. 단순히 함수를 값으로 쓰면 `enter`와 동일하게 동작하고, `enter`와 `exit` 두 단계로 나눌 수도 있다. 두 단계의 차이는 자식 노드 방문 여부인데, `enter` 시점에는 아직 자식을 방문하지 않은 상태라 현재 노드를 기준으로만 작업할 수 있다. 반면 `exit` 시점에는 모든 자식 방문이 끝난 뒤라서, 자식 처리 결과에 의존하는 변환이라면 반드시 `exit`를 써야 한다.

`path`와 `node`도 구분이 필요하다. 처음 Babel 코드를 봤을 때 왜 `node`를 직접 쓰지 않고 `path`를 통해 접근하는지 이해가 안 됐는데, `path`는 단순한 노드가 아니라 그 노드의 **위치 정보와 탐색 컨텍스트** 전체를 담은 객체다. `path.node`가 실제 AST 노드고, `path.parent`는 부모 노드, `path.scope`는 현재 스코프 정보를 담는다. `path.replaceWith()`, `path.remove()` 같은 메서드로 트리를 수정할 수 있는 것도 `path`가 탐색 컨텍스트를 갖고 있기 때문이다.

> Visitor 패턴은 탐색(어떻게 순회할 것인가)과 처리(무엇을 할 것인가)를 분리한다. 이 분리 덕분에 각 변환 플러그인은 자신이 관심 있는 노드 유형만 선언하면 된다.

AST를 직접 탐구해보고 싶다면 **astexplorer.net**이 가장 좋다. 좌측에 소스 코드를 입력하면 우측에서 파서별 AST 트리를 실시간으로 확인할 수 있다. Babel, acorn, TypeScript, swc 등 다양한 파서를 선택할 수 있고, 특정 노드를 클릭하면 소스에서 대응하는 위치가 하이라이트된다. 이 도구 없이 AST를 이해하려고 했다면 훨씬 오래 걸렸을 것이다.
...
2편에서는 번들러가 이 AST를 실제로 어떻게 활용하는지를 다룬다. 파싱 단계에서 어떤 파서를 쓰는지, 트리 셰이킹이 AST 수준에서 어떻게 동작하는지, 변환된 AST를 다시 소스 코드로 어떻게 만들어내는지. AST의 구조를 이해하면 번들러의 동작 방식도 훨씬 명확하게 보인다.
