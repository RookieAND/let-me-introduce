...
`gem-server` 에 새 컬렉션을 추가할 때마다 동일한 과정이 세트처럼 따라붙었다.
Schema 파일을 만들고, Repository 파일을 만들고, index.ts 에 임포트를 추가해야 했다.
이 세 단계가 매번 빠짐없이 따라붙었다.

처음 몇 번은 별 생각 없이 손으로 했다.
그런데 어느 순간부터 이건 내가 직접 해야 하는 작업이 아니라는 생각이 들었다.
명령어 하나로 이 과정이 끝난다면 그게 훨씬 낫겠다 싶었고, 그게 Code Generator 도입을 결정한 이유다.

이 글은 처음 살펴본 hygen 을 결국 선택하지 않고 PlopJS 를 택한 이유, 그리고 실제 Generator 를 구현한 과정을 기록한다.
...
먼저 **다운로드 및 업데이트 빈도**가 먼저 눈에 띄었다.
PlopJS 는 hygen 에 비해 약 3배 많은 다운로드 수를 기록하고 있었고, hygen 은 마지막 업데이트가 2년 전이었다. 솔직히... 조용히 묻히고 있는 느낌이었다.
...
솔직히 말하면, 나의 EJS 미숙으로 인한 선택이기도 했다.
