...
그 시절(내부에서 Jenkins3라 부르던)엔 빌드가 EC2 위에서 직접 실행됐고, Turbo Remote Cache 역시 API Token + HTTPS 기반의 공식 스펙을 구현한 HTTP 서버를 Docker로 별도 운영하면서 도메인으로 참조하는 구조였다.
...
이 때문에 빌드 프로세스가 종종 잘 돌아가다 강제로 종료되는 일이 번번히 일어났고, 이걸 겪은 입장으로서 솔직히 꽤 짜증스러웠다.
...
모듈 수가 매우 많은 서비스의 경우엔 **Node-Builder**가 추가로 올라온다.
...
실제로 돌려보니 Turbo가 EFS 캐시를 전혀 읽지 못했는데, Turbo가 캐시 디렉토리를 탐색하는 방식상 환경변수로 경로를 바꾸는 것만으로는 충분하지 않았던 것이다.
...
기존 `.turbo` 디렉토리를 먼저 제거하고 EFS 경로를 가리키는 심볼릭 링크를 생성하는 방식인데, 공식 환경변수가 아닌 우회 방식이기 때문에 Turbo 버전 업그레이드나 Dockerfile 구조 변경 시 캐시 연결이 조용히 끊길 수 있다는 게 트레이드오프로 남아 있다.
...
├── Layer: RUN ln -sf /mnt/.turbo /app/.turbo   ← EFS 심볼릭 링크 설정
...
## Jenkins3 vs Jenkins Internal: 실제로 얼마나 달라졌나

| 항목 | Jenkins3 (기존) | Jenkins Internal (현재) |
|---|---|---|
...
그간 구 버전 Jenkins Pipeline이 빠르게 느껴졌던 건 사실 운이었는데, 동일한 SPOT 인스턴스에 이전 빌드의 컨테이너 레이어가 남아있던 경우에만 캐시가 동작했고 인스턴스가 교체되면 캐시 자체가 사라졌다.
SPOT 회수 시 빌드가 중단되는 문제도 있었으니 그게 딱히 좋은 환경은 아니었다. (은근 짜증났다는 건 앞서 말했지만)

Jenkins Internal은 Kaniko 내 파일 시스템 기반 스냅샷 오버헤드(redo 모드)가 있어서 단순 비교 시 약 2분 느리게 측정될 수 있지만, 캐시가 EFS에 영속적으로 유지되기 때문에 Full Turbo 상태에서는 더 빠르고 안정적이다.
...
여기에 더해, EFS는 TTL이나 자동 만료 기능이 없어서 S3의 Lifecycle 정책처럼 일정 기간 이후 자동 삭제가 되지 않고, 사용되지 않는 캐시가 스토리지를 계속 점유하기 때문에 주기적으로 수동 정리를 해줘야 한다.
