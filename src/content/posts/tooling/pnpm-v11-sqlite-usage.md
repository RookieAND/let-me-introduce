...
먼저 `.mpk` 인덱스 파일 (과거에는 json 이었다) 을 열면 react를 구성하는 파일들의 content hash 목록이 나온다.
...
1. pnpm-lock.yaml 에서 읽은 패키지의 integrity 를 HEX 로 변환한다.
2. 변환된 HEX 값의 앞 두 자리를 `files` 내 디렉토리 중 하나로 지정하고, 그 안에 나머지로 파일명을 구성한다.

그런데 v11은 이러한 경로 변환을 더 이상 사용하지 않는다!
왜냐하면 `pnpm-lock.yaml` 에서 읽은 integrity 문자열을 변환 없이 SQLite key로 그대로 사용하기 때문이다.
...
실제 pnpm 내부에서 쓰이는 SQLite (index.db) 에 저장된 패키지 별 key 를 lockfile 과 비교하면 일치함을 알 수 있다.
...
기존에는 lockfile을 읽을 때마다 integrity를 HEX로 변환하고, 변환된 값으로 경로를 구성한 뒤 파일시스템을 탐색해야 했다.

하지만 v11은 이 두 단계를 모두 없앴다! 이제는 별도의 HEX 변환 없이 integrity를 key로 바로 쓰고, 파일시스템 탐색 대신 DB 기반의 B-tree 인덱스 조회로 처리하여 상당한 성능 향상을 꾀했다.
...
여기까지 글을 쓰면서 한 가지 중요한 걸 빠뜨렸다고 생각한 게 바로 SQLite 사용 방법이었다.
pnpm이 v11 부터 SQLite를 쓴다는 건 알겠는데 과연 내부적으로는 어떤 드라이버를 쓰는 걸까?
...
그렇다, 앞에 접두사를 보면 알 수 있지만 바로 Node.js 내장 모듈이다.
Node 에서 v22 부터는 별도의 의존성 없이 SQLite 를 사용할 수 있도록 모듈을 추가한 거다!
...
단순히 최신 Node.js 기능을 쓰고 싶어서 요구사항을 올린 게 아니었다. `node:sqlite`를 안정적으로 사용할 수 있는 최소 버전이 바로 여기에 있었다. (어쩐지 괜히 버전을 올리라고 하더라니)

### 실제 스키마를 기반으로 저장 방식을 파악해보자.
...
위 설정에서는 512MB mmap과 32MB 페이지 캐시가 눈에 띈다.
개인적으로는 install 과정에서 패키지를 조회할 때 DB 전체를 OS 파일 캐시에 올려두는 전략을 사용했구나~ 라고 이해했다.

### 배치 쓰기

pnpm v11 은 fetch 단계에서 패키지 인덱스를 기록할 때 매번 개별 트랜잭션을 열지 않는다.
그 대신 `queueWrites()` + `process.nextTick()`으로 쓰기를 모아서 단일 트랜잭션으로 Flush 하는 방식을 쓴다.

병렬 fetch가 진행되는 동안 각각의 쓰기가 큐에 쌓이고, 이벤트 루프 다음 틱에서 `BEGIN IMMEDIATE ... COMMIT`으로 한번에 처리된다.
왜 이렇게 배치로 처리할까 싶었는데, SQLite는 트랜잭션당 오버헤드가 있기 때문에 배치로 처리해 쓰기 횟수 자체를 줄이는 게 더 이득이라고 판단했다는 PR 코멘트를 보았다.

### v10 → v11: 기존 캐시는 어떻게 되는가

pnpm v11 소스 코드를 보면서 하나 더 확인하고 싶었던 점이 있다.
만약 버전을 v10에서 v11로 올리면 기존에 쌓인 캐시는 어떻게 처리되는가?

결론은 생각보다 단순했는데, 기존 버전에 쓰이던 파일은 그냥 쓰이지 않고 일괄적으로 버려진다.

스토어 경로 자체가 `v10/`에서 `v11/`로 바뀌기 때문에, 기존 `.mpk` 인덱스를 SQLite로 자동 변환하는 마이그레이션 코드가 없다. 그렇기 때문에 v11 첫 실행 시 인덱스에 없는 패키지는 그냥 다시 다운로드한다. 
따라서 CI에서 스토어 캐시를 사용하고 있다면 캐시 경로를 v11에 맞게 업데이트해야 한다! 그러지 않으면 v11로 올린 직후 캐시 히트가 전혀 없는 상황이 되어서 곤란하다. (아니면 경로를 별도로 맞춰도 된다)
...
특히 CI 환경에서 효과가 크다고 생각했다. (Fetch 단계에서 인덱스 탐색 비용 대폭 감소!)
왜냐하면 캐시를 복원한 후 실행하는 install에서 인덱스 탐색 비용이 대폭 줄어들기 때문이다. 
로컬 환경에서도 각각의 레포지토리가 크거나 많아 CAS 내 패키지가 오래 누적된 경우라면 체감 가능한 수준이다.

---

## 마치며

pnpm v11의 SQLite 전환은 단순한 구현 변경이 아니리, "수백만 개의 작은 파일"이라는 근본적인 병목을 "하나의 잘 설계된 데이터베이스"로 대체한 아키텍처 결정이라고 생각한다.

WAL 모드로 병렬 쓰기를 허용하고, 번들 매니페스트로 불필요한 파일 파싱을 제거하고, integrity를 그대로 SQLite key로 써서 base64 변환 비용을 없앤다. 각각의 요소는 작아 보일지 몰라고 이 결정들이 모여 install 속도를 실질적으로 높혔다.

패키지 매니저를 쓰는 입장에서는 그냥 빨라진 install을 경험할 뿐이다. 하지만 그 안에서 어떤 설계 결정이 내려졌는지를 알면, 평범한 `pnpm install` 명령어가 다르게 보이기 시작했다. 이런 디테일함이 있었다니... 참 어렵다