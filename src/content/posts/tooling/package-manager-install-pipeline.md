...
Resolution → Fetch → Link, 이 세 단계를 이해하고 나면 패키지 매니저의 동작이 블랙박스에서 벗어난다.

Resolution은 락파일이 왜 필요한지를 설명해준다. 매번 레지스트리를 조회해 버전을 다시 계산하는 대신, 이미 계산된 결과를 재사용하기 위해서다.
Fetch는 왜 두 번째 install이 빠른지를 설명해준다. 글로벌 캐시에 integrity 기반으로 저장해두고 재사용하기 때문이다.
Link는 pnpm과 Yarn Berry가 왜 다른지를 설명해준다. "파일을 어떻게 프로젝트에 연결할 것인가"에 대한 철학의 차이다.

앞으로 `yarn install`이나 `pnpm install` 로그를 볼 때, 각 단계에서 무슨 일이 일어나는지 조금 더 구체적으로 그려질 것이다.
