...
### local Variable doesn't trigger re-render
...
- 설령 alert 함수가 리렌더링 이후에 실행되더라도, 해당 작업이 스케줄러에 등록될 당시의 state 값은 **리렌더링이 일어나기 전의 값**이기에 이런 현상이 발생하는 것이다.
